// Companion Discord MCP Bridge v4.0
...
// ============ VOICE CONFIG ============
// Add your companion's ElevenLabs voice ID here when ready
const VOICE_MAP = {
  companion: "", // <-- DROP YOUR ELEVENLABS VOICE ID HERE
};
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        voice: { type: "string", description: 'Voice to use: "companion" (default)', default: "companion" },
      },
  ...
  try {
    switch (name) {
      ...
      case "discord_send_voice": {
        const voiceName = (args.voice || "companion").toLowerCase();
        const voiceId = VOICE_MAP[voiceName] || VOICE_MAP.companion;

        if (!voiceId) {
          return {
            content: [{ type: "text", text: "Voice not configured yet — add your companion's ElevenLabs voice ID to the VOICE_MAP in the worker." }],
            isError: true,
          };
        }
  ...
  if (body.method === "initialize") {
    return {
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "discord-mcp-bridge", version: "4.1.0" },
      },
    };
  }
    ...
    ctx.waitUntil(
      (async () => {
        try {
          ...
          await send("endpoint", `${reqUrl.origin}${reqUrl.pathname}?session=${sessionId}`);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/" || url.pathname === "") {
      return new Response(
        JSON.stringify({ name: "discord-mcp-bridge", version: "4.1.0", status: "online" }),
        { headers: { "Content-Type": "application/json", ...corsHeaders() } }
      );
    }

    const [route, providedSecret] = url.pathname.split("/").filter(Boolean);

    if (route === "mcp" || route === "sse") {
      // Auth: URL path secret, e.g. /sse/<MCP_SECRET>. Refuses everything
      // (including legacy /sse and /mcp) until MCP_SECRET is configured.
      if (!env.MCP_SECRET || providedSecret !== env.MCP_SECRET) {
